""" Detect change on a pair various different ways, then assemble
output into tiled image composite.  The main program saves the input images
as a composite and then the outputs as a composite.

The code that follows is based on bulk_detect.py.
...
External function: detect_change()
    ...
    Output: An image array with change points displayed.
...
def detect_change(im1_file,
    ...
    inputs_tiled_filename = '-'.join([file_base,
                                    date1,
                                    date2,
                                    'inputstiled.jpg'])
    outputs_tiled_filename = '-'.join([file_base,
                                    date1,
                                    date2,
    ...
    logfile.write('Found {0} kps in im1\n'.format(len(kps1)))
    print 'Found {0} kps in im2'.format(len(kps2))
    logfile.write('Found {0} kps in im2\n'.format(len(kps2)))
...
# Use this instead for relaxed matching:
    ...
    logfile.write('Found {0} match candidates...\n'.format(len(match_candidates)))
    ...
    for knnlist in match_candidates:
        for m in knnlist:
            if _are_close(kps1[m.queryIdx],kps2[m.trainIdx],
                ...
                matches.append(m)
                break
    print '...of which {0} are within the proximity limit of {1} pixels.\n'.format(len(matches),proximity_limit)
    logfile.write('...of which {0} are within the proximity limit of {1} pixels.\n'.format(len(matches),proximity_limit))
...
# first do forward direction
    ...
    logfile.write('Found {0} change keypoints (forward direction)\n'.format(len(kps2_changed)))
...
# repeat above for backward direction
    ...
    logfile.write('Found {0} change keypoints (backward direction)\n'.format(len(kps1_changed)))
...
if __name__ == '__main__':
    ...
    """
    Example: $ python tile.py
        '../dim1000test/Candlestick-2009.jpg'
        '../dim1000test/Candlestick-2012.jpg'
        '../dim1000test'
    """
        
    KAZE_THRESHOLD = 1e-07 # To adjust by hand
    SIFT_THRESHOLD = 2e-03
    
    image_dir = sys.argv[3]  # dir name should not include trailing /
    im1 = sys.argv[1].split(image_dir+'/')[1]
    im2 = sys.argv[2].split(image_dir+'/')[1]

    file_base = im2.split('-')[0]
    date1 = im1.split('-')[1].split('.jpg')[0]
    date2 = im2.split('-')[1].split('.jpg')[0]
    inputs_tiled_filename = '-'.join([file_base,
                                    date1,
                                    date2,
                                    'inputstiled.jpg'])
    outputs_tiled_filename = '-'.join([file_base,
                                    date1,
                                    date2,
                                    'KAZEthresh{:.0e}'.format(KAZE_THRESHOLD),
                                    'SIFTthresh{:.0e}'.format(SIFT_THRESHOLD),
                                    'tiled.jpg'])
    logfile = open(outputs_tiled_filename.split('-tiled.jpg')[0]+'.log','a')
    logfile.write('tile.py run ' + time.strftime('%c') + '\n')
    
    matchratio_im = detect_change(im1,im2,logfile,RelDir=image_dir,
                            FEATURES='SIFT',
                            KNEAREST=1,
                            STATS_TEST='MATCHKPRATIO',
                            MATCH_PROBABILITY_THRESHOLD=SIFT_THRESHOLD)
    sift_im = detect_change(im1,im2,logfile,RelDir=image_dir,
                            FEATURES='SIFT',
                            KNEAREST=1,
                            STATS_TEST='PBYKPRATE',
                            MATCH_PROBABILITY_THRESHOLD=SIFT_THRESHOLD)
    kaze_im = detect_change(im1,im2,logfile,RelDir=image_dir,
                            FEATURES='KAZE',
                            KNEAREST=1,
                            STATS_TEST='PBYKPRATE',
                            MATCH_PROBABILITY_THRESHOLD=KAZE_THRESHOLD)
    kazekNN_im = detect_change(im1,im2,logfile,RelDir=image_dir,
                            FEATURES='KAZE',
                            KNEAREST=5,
                            STATS_TEST='PBYKPRATE',
                            MATCH_PROBABILITY_THRESHOLD=KAZE_THRESHOLD)

    input_ims = np.concatenate((cv2.imread(sys.argv[1]),
                                cv2.imread(sys.argv[2])),axis=1)
    output_ims = np.concatenate((matchratio_im,sift_im,kaze_im,kazekNN_im),
                                axis=1)
    cv2.imwrite(inputs_tiled_filename,input_ims)
    cv2.imwrite(outputs_tiled_filename,output_ims)
    logfile.close()
